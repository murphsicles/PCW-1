...
## State Machine Diagram Description (from image)

Global timers & flags
τ_hold = maximum queued time allowed
τ_period = periodic re-announce
d_confirm = confirmation depth
expires_at_policy = expires_at_policy
expires_at_invoice = expires_at_invoice
supersedes flag = indicates old raw bytes must not be broadcast
voided_offchain - Indicates old raw bytes must not be broadcast

Per-note state machine

S0 Constructed -> S1 Signed -> S2 Queued -> S3 Broadcast -> S4 Confirmed -> S Rebroadcast -> reissue ~ S3
retissue(i) -> reissue
Diterministic identital ogs + timers yield identitions.

Invoice-level state machine

I0 Open
LOseft (insufficient data)
I1 Fan-Out Pending -> Insufficient-Utxo ()
TE Expired (<TS)
I2 Ready (completed)
IF exseded
I3 Building
...
superseded (f)
I4 Closing
Expiry Lov id all queued or passive monitor until confirmed
Illustrative only

Scheduler tick: (canonical Ison')
"reissue-record":
"invoice-hash":
"ilante.id":
"event":
...
A) Insufficient-UTXOs: fan-out once
It allowed else fail;
B) Fee change: reissue affected.
C) Conflicting spend: abort+rebuild reservation table R
D) Reorg: orphan then rebroadcast same txbytes T,
E) Expiry, void all queued on
passive monitor until confirmed.

(Full image description: The diagram shows global timers, per-note and invoice-level state machines, scheduler tick JSON, record examples for reissue/superseded/cancel/conflict/orphaned, and failure behaviors A-E.)
