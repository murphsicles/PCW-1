...
impl NoteState {
    ...
    pub fn transition(&self, event: Event) -> Result<Self, PcwError> {
        match (self, event) {
            ...
            (s, Event::ExplicitCancel) if matches!(s, NoteState::Constructed | NoteState::Signed | NoteState::Queued | NoteState::Broadcast | NoteState::Seen) => Ok(NoteState::Cancelled),
...
impl InvoiceState {
    ...
    pub fn transition(&self, event: Event) -> Result<Self, PcwError> {
        match (self, event) {
            ...
            (s, Event::ExplicitCancel) if matches!(s, InvoiceState::Open | InvoiceState::FanOutPending | InvoiceState::Building | InvoiceState::Ready | InvoiceState::Broadcasting) => Ok(InvoiceState::Stopped),
