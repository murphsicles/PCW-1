...
fn ecdh_z(my_priv: &[u8; 32], their_pub: &PublicKey) -> Result<[u8; 32], PcwError> {
    ...
    Ok(shared_point.serialize()[1..33].try_into().map_err(|_| PcwError::Other("Invalid ECDH point".to_string()))?)
}
...
pub async fn handshake(
    ...
    stream.write_all(&my_pub).await.map_err(|e| PcwError::Other(format!("Failed to send public key: {}", e)))?;

    // Read their public key
    let mut their_pub_buf = [0u8; 33];
    stream.read_exact(&mut their_pub_buf).await.map_err(|e| PcwError::Other(format!("Failed to read public key: {}", e)))?;
...
pub async fn exchange_policy(
    ...
    if let Some(p) = policy {
        ...
        stream.write_all(&len).await.map_err(|e| PcwError::Other(format!("Failed to send policy length: {}", e)))?;
        stream.write_all(&bytes).await.map_err(|e| PcwError::Other(format!("Failed to send policy: {}", e)))?;
        Ok(p)
    } else {
        let mut len_buf = [0u8; 4];
        stream.read_exact(&mut len_buf).await.map_err(|e| PcwError::Other(format!("Failed to read policy length: {}", e)))?;
        let len = u32::from_le_bytes(len_buf) as usize;
        let mut bytes = vec![0u8; len];
        stream.read_exact(&mut bytes).await.map_err(|e| PcwError::Other(format!("Failed to read policy data: {}", e)))?;
...
pub async fn exchange_invoice(
    ...
    if let Some(inv) = invoice {
        ...
        let len = (bytes.len() as u32).to_le_bytes();
        stream.write_all(&len).await.map_err(|e| PcwError::Other(format!("Failed to send invoice length: {}", e)))?;
        stream.write_all(&bytes).await.map_err(|e| PcwError::Other(format!("Failed to send invoice: {}", e)))?;
        Ok(inv)
    } else {
        let mut len_buf = [0u8; 4];
        stream.read_exact(&mut len_buf).await.map_err(|e| PcwError::Other(format!("Failed to read invoice length: {}", e)))?;
        let len = u32::from_le_bytes(len_buf) as usize;
        let mut bytes = vec![0u8; len];
        stream.read_exact(&mut bytes).await.map_err(|e| PcwError::Other(format!("Failed to read invoice data: {}", e)))?;
...
mod tests {
    ...
    async fn test_handshake_symmetry() -> Result<(), PcwError> {
        let listener = TcpListener::bind("127.0.0.1:0").await.map_err(|e| PcwError::Other(format!("Bind failed: {}", e)))?;
        let addr = listener.local_addr().map_err(|e| PcwError::Other(format!("Get local addr failed: {}", e)))?;
        let t1 = tokio::spawn(async move {
            let (mut stream, _) = listener.accept().await.map_err(|e| PcwError::Other(format!("Accept failed: {}", e)))?;
            let priv_k1 = [1; 32];
            let key1 = IdentityKeypair::new(priv_k1)?;
            handshake(&mut stream, &key1).await
        });
        let t2 = tokio::spawn(async move {
            let mut stream = TcpStream::connect(addr).await.map_err(|e| PcwError::Other(format!("Connect failed: {}", e)))?;
            let priv_k2 = [2; 32];
            let key2 = IdentityKeypair::new(priv_k2)?;
            handshake(&mut stream, &key2).await
        });
        let z1 = t1.await.map_err(|e| PcwError::Other(format!("Task 1 failed: {}", e)))??;
        let z2 = t2.await.map_err(|e| PcwError::Other(format!("Task 2 failed: {}", e)))??;
    ...
    async fn test_exchange_policy() -> Result<(), PcwError> {
        ...
        let p1 = t1.await.map_err(|e| PcwError::Other(format!("Task 1 failed: {}", e)))??;
        let p2 = t2.await.map_err(|e| PcwError::Other(format!("Task 2 failed: {}", e)))??;
    ...
    async fn test_exchange_invoice() -> Result<(), PcwError> {
        ...
        let i1 = t1.await.map_err(|e| PcwError::Other(format!("Task 1 failed: {}", e)))??;
        let i2 = t2.await.map_err(|e| PcwError::Other(format!("Task 2 failed: {}", e)))??;
