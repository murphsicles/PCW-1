use pcw_protocol::{PcwError, bounded_split, ecdh_z, build_reservations, build_note_tx, compute_leaves, merkle_root, generate_proof, verify_proof};
use pcw_protocol::keys::{IdentityKeypair, AnchorKeypair};
...
use pcw_protocol::receipts::{Entry, Manifest};
...
fn test_full_protocol_flow() -> Result<(), PcwError> {
    ...
    let u0 = vec![
        Utxo {
            ...
            value: 1500,
            script_pubkey: mock_script.to_bytes().to_vec(),
        },
        Utxo {
            outpoint: OutPoint {
